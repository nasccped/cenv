...
/* This file defines common utilities for cenv runtime, such as print_erro_tag,
 * string mode checkers and so on... */
...
/* Print an error tag to stderr (automatically). Note that this function
 * doesn't automatically adds newline at the end. You must handle it manually!
 */
void print_error_tag(const char *restrict fmt, ...);

/* Print a note tag to std(out/err) (decided by using the 'is_err' param). Note
 * that this function doesn't automatically adds newline at the end. You must
 * handle it manually! */
...
/* Returns if the provided char pointer refers to a recognizable program flag
 * (such as --help,
 * --version, ...). Note that this function will also returns '0' if s refers
 * to NULL or zero-length string. */
int string_is_program_flag(char *s);

/* Returns if the provided char pointer refers to NULL / zero-length /
 * whitespace only string. */
int string_is_whitespace(char *s);

/* Apply ansi escape styling to a given dest char pointer. This function also
 * returns the dest address.
 *
 * WARN: dest and source pointer shouldn't be the same address (even if enough
 * size). This function works with 'sprintf' function and passing 'sprintf(s,
 * "%s...", s, ...)' can leads to undefined behavior. */
